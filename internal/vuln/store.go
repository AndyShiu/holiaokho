...
func List(ctx context.Context, c *content.Service, f Filter) ([]Finding, int, error) {
	...
	q := `SELECT pv.package_id, r.name, r.format, p.namespace, p.name, p.version,
			v.id, v.aliases, v.summary, v.severity, v.score::float8, pv.fixed_in, v.published, pv.first_seen ` + from +
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.PackageID, &x.Repository, &x.Format, &x.Namespace, &x.Name, &x.Version,
			&x.VulnID, &x.Aliases, &x.Summary, &x.Severity, &x.Score, &x.FixedIn, &x.Published, &x.FirstSeen); err != nil {
			return nil, 0, err
		}
		out = append(out, x)
	}
