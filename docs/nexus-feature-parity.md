...
> 狀態欄：`P0`–`P7` 對應 `holiaokho-architecture-draft.md` §14 的實作階段；`後` = 有排但無階段；`不做` = 明確不做並附理由。
> 2026-09-13 初版。
...
| 功能 | Nexus | Holiaokho |
|---|---|---|
| hosted / proxy / group | ✅ | **P1** 起每格式都做三種 |
| Proxy：contentMaxAge / metadataMaxAge | ✅ | **P1** |
| Proxy：negative cache + TTL | ✅ | **P1** |
| Proxy：上游認證（Basic / Bearer / preemptive） | ✅ | **P1** |
| Proxy：自訂 CA / trust store、outbound HTTP proxy、連線逾時、重試、autoBlock | ✅ | **P1** |
| Proxy：blocked（暫停上游） | ✅ | **P1** |
| Group：成員排序、first-match、metadata merge | ✅ | **P1** |
| Group：deploy 到 group（轉發到第一個 hosted） | ✅ | **P1** |
| Hosted：writePolicy（ALLOW / ALLOW_ONCE / DENY） | ✅ | **P1** |
| Maven：layoutPolicy STRICT/PERMISSIVE、versionPolicy RELEASE/SNAPSHOT/MIXED、contentDisposition | ✅ | **P1** |
| Maven：snapshot 時間戳版本、`maven-metadata.xml` 產生與 merge、checksum 旁邊檔 | ✅ | **P1** |
| Maven：rebuild metadata 任務、maven-indexer | ✅ | **P1** / 後 |
| Docker：httpPort / httpsPort connector、subdomain connector、path 模式 | ✅ | **P3** |
| Docker：v1 API | ✅（預設關） | 不做（Docker 已棄用） |
| Docker：forceBasicAuth、Bearer token realm | ✅ | **P3** |
...
|---|---|---|
| File blob store | ✅ | **P0** |
| S3 blob store（含 IAM role、encryption、prefix） | ✅ | **P4** |
| Azure Blob store | ✅ | 後 |
...
| Blob store 容量／指標 | ✅ | **P4** |
| 跨 repo 去重（content-addressable） | ❌（Nexus 不去重） | **P0** 就內建，**超越 Nexus** |
...
| 功能 | Nexus | Holiaokho |
|---|---|---|
| Local users（bcrypt/Shiro） | ✅ | **P0** |
| Roles / privileges（repo-view、repo-admin、application、wildcard、script） | ✅ | **P0** 基礎、**P5** 完整 |
| Anonymous access 開關 + 匿名角色 | ✅ | **P0** |
| LDAP / Active Directory | ✅ | **P5** |
| SAML | Pro | **P5**（我們做 OIDC，另評估 SAML） |
| OIDC | ❌（Nexus 沒有） | **P5**，**超越 Nexus** |
| User tokens（給 CI 的長效 token） | Pro | **P5**，**超越 Nexus CE** |
| Docker Bearer token realm | ✅ | **P3** |
| npm Bearer token realm（`npm login`） | ✅ | **P2** |
...
| 登入失敗限流（連續失敗回 429） | ✅ | **P0**（實測 3 次失敗即觸發） |
...
| 任務 | Nexus | Holiaokho |
|---|---|---|
| Cleanup policies 執行（依 lastDownloaded / lastBlobUpdated / regex / 保留 N 版） | ✅ | **P4** |
...
| Repair - Rebuild Maven repository metadata | ✅ | **P1** |
...
| Cleanup unused `<format>` blobs（每格式一個，內建自動排程） | ✅ | **P4**（我們的 blob GC 統一做） |
...
| REST API（`/service/rest/v1`） | ✅ | **P0** 自有 `/api/v1`；Nexus 相容子集 **P7** |
...
| System status / health check | ✅ | **P0** `/healthz` `/readyz` |
...
| Metrics（Prometheus） | ✅（`/service/metrics/prometheus`） | **P0** `/metrics` |
...
| Nexus 升級／migration 工具 | ✅ | **P0** `import-nexus` |
...
| 功能 | 規劃 |
|---|---|
| CE 上限：100,000 components、每日 200,000 requests | **無上限**，P0 就是 |
| User tokens | **P5** |
| SAML | 後 |
